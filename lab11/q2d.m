...
for i= 1:1
    ...
    nx = size(xd, 2);
    ny = size(yd, 2);
    str = strcat('problem(d) h = k = ', num2str(h));
...
title('loglog plot: \Delta x(=\Delta y) vs max error, 5-point stencil (Gauss Siedel) for problem (d)');
saveas(gcf,sprintf('q1_df%d.png',fig));
fig = fig+1;
...
xlabel('\Delta x(=\Delta y)');
ylabel('Max Absolute Error');
title('loglog plot: \Delta x(=\Delta y) vs max error, 5-point stencil (Jacobi) for problem (d)');
saveas(gcf,sprintf('q1_df%d.png',fig));
fig = fig+1;
...
function [max_errorg, max_errorj] = five_point_stencil_gaussjacobi(h, xd, yd, nx, ny, f, ex, eqx1, eqx2, eqy1, eqy2)
    ...
    while(it<1e5)
        for i=2:nx-1
            for j=2:ny-1
                tabg(i, j) = (1/(-4+h*h))*(B((i-1)*ny+j)-(1-h/2)*tabg(i-1, j)-(1-h/2)*tabg(i, j-1)-(1+h/2)*tabg(i, j+1)-(1+h/2)*tabg(i+1, j));
                tabj2(i, j) = (1/(-4+h*h))*(B((i-1)*ny+j)-(1-h/2)*tabj1(i-1, j)-(1-h/2)*tabj1(i, j-1)-(1+h/2)*tabj1(i, j+1)-(1+h/2)*tabj1(i+1, j));
            end
        end
        for i=2:nx-1
            for j=2:ny-1

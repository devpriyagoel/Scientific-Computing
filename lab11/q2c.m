...
title('loglog plot: \Delta x(=\Delta y) vs max error, 5-point stencil (Gauss Siedel) for problem (c)');
saveas(gcf,sprintf('q2_cf%d.png',fig));
fig = fig+1;
...
xlabel('\Delta x(=\Delta y)');
ylabel('Max Absolute Error');
title('loglog plot: \Delta x(=\Delta y) vs max error, 5-point stencil (Jacobi) for problem (c)');
saveas(gcf,sprintf('q2_cf%d.png',fig));
fig = fig+1;
h=2e-2;
for i= 1:1
    ...
    nx = size(xd, 2);
    ny = size(yd, 2);
    str = strcat('problem(c) h = k = ', num2str(h));
...
%         B(i) = eqy1(yd(i)); 
    ...
    while(it<1e5)
        for i=1:ny-1
            tabg(1, i) = (eqy1(yd(i))*h-tabg(2, i))/(h-1);   
            tabj2(1, i) = (eqy1(yd(i))*h-tabj1(2, i))/(h-1);    
        end
        for i=2:nx-1
            for j=2:ny-1
                tabg(i, j) = (1/(-4+h*h))*(B((i-1)*ny+j)-tabg(i-1, j)-tabg(i, j-1)-tabg(i, j+1)-tabg(i+1, j));
                tabj2(i, j) = (1/(-4+h*h))*(B((i-1)*ny+j)-tabj1(i-1, j)-tabj1(i, j-1)-tabj1(i, j+1)-tabj1(i+1, j));
            end
        end
        for i=1:nx
            for j=1:ny

...
eqx2 = @(x)x*x*0.5;
eqy1 = @(y)sin(pi*y);
eqy2 = @(y)exp(pi)*sin(pi*y)+y*y*0.5;
a=0;
b=1;
h = 2e-1;
ex = @(x, y)exp(pi*x)*sin(pi*y)+(x*x*y*y)*0.5;
...
title('loglog plot: \Delta x(=\Delta y) vs max error, 5-point stencil (Gauss Siedel) for problem (b)');
saveas(gcf,sprintf('q2_bf%d.png',fig));
fig = fig+1;
...
xlabel('\Delta x(=\Delta y)');
ylabel('Max Absolute Error');
title('loglog plot: \Delta x(=\Delta y) vs max error, 5-point stencil (Jacobi) for problem (b)');
saveas(gcf,sprintf('q2_bf%d.png',fig));
fig = fig+1;
h=2e-2;
for i= 1:1
    ...
    nx = size(xd, 2);
    ny = size(yd, 2);
    str = strcat('problem(b) h = k = ', num2str(h));

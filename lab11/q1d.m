...
title('loglog plot: \Delta x(=\Delta y) vs max error by five-point stencil for problem (d)');
...
function max_error = five_point_stencil_matrixf(h, xd, yd, nx, ny, f, ex, eqx1, eqx2, eqy1, eqy2, str)
    ...
    for i=2:nx-1
        for j=2:ny-1
            A((i-1)*ny+j, (i-1)*ny+j) = -4+h*h;
            A((i-1)*ny+j, (i-1)*ny+j+1) = 1+h/2;
            A((i-1)*ny+j, (i)*ny+j) = 1+h/2;
            A((i-1)*ny+j, (i-1)*ny+j-1) = 1-h/2;
            A((i-1)*ny+j, (i-2)*ny+j) = 1-h/2;
